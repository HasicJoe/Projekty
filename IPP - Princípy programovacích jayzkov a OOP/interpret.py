""" IPP - Projekt 2. - Samuel Valaštín <xvalas10@stud.fit.vutbr.cz> """
import os
import sys
import argparse
...
def defvar(model,instruction):    
    ...
    if re.match("(^[T|L|G]{1}[F]{1}[@]{1}[a-zA-Z_\-$&%*!?][a-zA-Z0-9_\-$&%*!?]{0,}$)",variable.name_or_value):
        split_var = re.split("@",variable.name_or_value)
...
def move(model,instruction):
    ...
    if source_var is None: # symb <string,int,bool,nil>
        if instruction.arguments[1].is_string():
            instruction.arguments[1].validate_string_value()
            ...
            instruction.arguments[1].validate_bool_value()
            destiny_var.update_value_type(instruction.arguments[1].name_or_value,instruction.arguments[1].type)    
        elif instruction.arguments[1].is_nil():
            instruction.arguments[1].validate_nil_value()
            destiny_var.update_value_type(instruction.arguments[1].name_or_value,instruction.arguments[1].type)      
    else: 
...
def conditional_jump(model,instruction,code,op):
    ...
    if op1 is None and op2 is None:
        ...
        if op1_type == "var":
            sys.exit(Exit_Codes.INVALID_VAR_ACCESS_ERR)
        op2_type,op2_value = op2.get_variable_type_value()
        if op2_value is None:
            sys.exit(Exit_Codes.INVALID_VAR_VALUE_ERR)
        if op2_type is None:
            sys.exit(Exit_Codes.INVALID_OP_TYPES_ERR)
    elif op1 is not None and op2 is None:
        op1_type,op1_value = op1.get_variable_type_value()
        if op1_value is None:
            sys.exit(Exit_Codes.INVALID_VAR_VALUE_ERR)
        if op1_type is None:
            sys.exit(Exit_Codes.INVALID_OP_TYPES_ERR)
        op2_type,op2_value = instruction.arguments[2].get_inst_data_without_position()
        ...
        if op2_value is None or op1_value is None:
            sys.exit(Exit_Codes.INVALID_VAR_VALUE_ERR)
...
def start_interpretation(code,args):
   # """TODO: Check argument len for without argument opcode!""""
    model = Memory_Model()
    ...
    while True:
        ...
        if instruction is not None:
            ...
            #print(model.instruction_name,model.position)
            if model.instruction_name == "MOVE": #done
                move(model,instruction)  
            elif model.instruction_name == "CREATEFRAME": #probably done - have to check it ! 
                model.create_TF_frame()
            elif model.instruction_name == "PUSHFRAME": # probably done - have to check it !
                model.push_TF_to_LF_frame()
            elif model.instruction_name == "POPFRAME":  # probably done
                model.pop_LF_to_TF_frame()
            elif model.instruction_name == "DEFVAR": #done
                defvar(model,instruction)
            elif model.instruction_name == "CALL": # probably done
                call(model,instruction,code)
            elif model.instruction_name == "RETURN":    #probably done
                return_handling(model,instruction,code)
            elif model.instruction_name == "PUSHS": #done - added new class to handle dataframe
                push_symb(model,instruction)
            elif model.instruction_name == "POPS":  # done
                pop_var(model,instruction)
            elif model.instruction_name == "ADD": #done
                arith(model,instruction,"+")
            elif model.instruction_name == "SUB" : #done
                arith(model,instruction,"-")
            elif model.instruction_name == "MUL": #done
                arith(model,instruction,"*")
            elif model.instruction_name == "IDIV": #done
                arith(model,instruction,"/")
            elif model.instruction_name == "LT":    #done
                compare(model,instruction,"LT")
            elif model.instruction_name == "GT":    #done
                compare(model,instruction,"GT")
            elif model.instruction_name == "EQ":    #done
                compare(model,instruction,"EQ")
            elif model.instruction_name == "AND": #done
                logic(model,instruction,"AND")
            elif model.instruction_name == "OR": #done
                logic(model,instruction,"OR")
            elif model.instruction_name == "NOT": #done
                not_ins(model,instruction)
            elif model.instruction_name == "INT2CHAR": #done
                int2char(model,instruction)
            elif model.instruction_name == "STRI2INT": # done
            ...
            elif model.instruction_name == "CONCAT": #done
                concat(model,instruction)
            elif model.instruction_name == "STRLEN": # done
                strlen(model,instruction)
            elif model.instruction_name == "GETCHAR": #done
                getchar(model,instruction)
            elif model.instruction_name == "SETCHAR": #done
                setchar(model,instruction)
            elif model.instruction_name == "TYPE": # done
                get_var_type(model,instruction)   
            elif model.instruction_name == "LABEL": #probably done
                continue
            elif model.instruction_name == "JUMP": #probably done
                jump(model,instruction,code)
            elif model.instruction_name == "JUMPIFEQ": #probably done
                conditional_jump(model,instruction,code,"EQ")
            elif model.instruction_name == "JUMPIFNEQ": # probably done
                conditional_jump(model,instruction,code,"NEQ")
            elif model.instruction_name == "EXIT": #done
                exit_it(model,instruction)
            elif model.instruction_name == "BREAK": #TODO
                break_debug(model,instruction)
            elif model.instruction_name == "DPRINT": # TODO
                dprint(model,instruction)  
            elif model.instruction_name == "CLEARS": #done
                model.clears()
            elif model.instruction_name == "ADDS": #done
                model.stack_arith("+")
            elif model.instruction_name == "SUBS": # done
                model.stack_arith("-")
            elif model.instruction_name == "MULS": #done 
                model.stack_arith("*")
            elif model.instruction_name == "IDIVS": #done
                model.stack_arith("/")
            elif model.instruction_name == "LTS": #done
                model.stack_comp("<")
            elif model.instruction_name == "GTS": #done
                model.stack_comp(">")
            elif model.instruction_name == "EQS": #done
                model.stack_comp("=")
            elif model.instruction_name == "ANDS": # done 
                model.stack_logic("A")
            elif model.instruction_name == "ORS": # done
                model.stack_logic("O")
            elif model.instruction_name == "NOTS": #done
                model.neg()
            elif model.instruction_name == "INT2CHARS": #done
                model.i2ch()
            elif model.instruction_name == "STRI2INTS": #done
                model.s2i()
            elif model.instruction_name == "JUMPIFEQS": #done
                stack_jump(model,instruction,"EQ",code)
            elif model.instruction_name == "JUMPIFNEQS": #done
...
def check_code(code,args):
    """ Function check if the data from XML file are valid(order,opcode...)"""
                ...
                if "type" in argument.attrib:
                    if argument.attrib["type"] in known_types:
                        if argument.text is not None: 
                            # replace escape sequencies 
                            if re.search("([\\\\][0-9]{3})",argument.text):
                                escape_sequencies = re.findall("([\\\\][0-9]{3})",argument.text)
