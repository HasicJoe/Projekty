...
def parse_people(df,names,months,years,writer):
    ...
    for this_emp_this_mon in mega_set_mix:
        ...
        for i in this_emp_this_mon.index:
           # print("\n*************************\n",this_emp_this_mon["Dátum výkonu"][i],"\n**************************\n")
        ...
        one_row = [meno_vodica,mes,pocet_stop_rozvoz,pocet_stop_zvoz,hmotnost_dor_zas,hmotnost_zvez_zas,pocet_dor_stop,pocet_zvez_stop,sum_rozvoz_zvoz,round(usp_rozvoz,0),round(usp_zvoz,0)]
...
def multiple_dfs(df_list, sh_name, file_name, spaces,col_len,l_rozvoz,l_zvoz,l_months):
    head_day = ["Deň","Počet rozvozov","Hmotnosť rozvoz","Počet zvozov","Hmotnosť zvoz", "Σ (Rozvoz + zvoz)"]
    head_month = ["Mesiac","Počet rozvozov","Hmotnosť rozvoz","Počet zvozov","Hmotnosť zvoz", "Σ (Rozvoz + zvoz)"]
    writer = pandas.ExcelWriter(file_name,engine='xlsxwriter')
    workbook  = writer.book   
    bar["value"] = 45
    header_fmt = workbook.add_format({'font_name': 'Arial', 'font_size': 10, 'bold': True,'bg_color': '#303030'})
    row = 0
    ...
    text_format = workbook.add_format({'text_wrap': True,"bold": True, 'font_color': '#f1f1f1',"bg_color":"#303030","valign":"vcenter","align":"center"})
    ...
    worksheet.insert_chart("H"+second_graph_posit,chart2,{'x_offset': 20, 'y_offset': 10,'x_scale': 1.4, 'y_scale': 1.2})
    return writer
...
def write_depo(depo_day,depo_month,list_rozvoz,list_zvoz,list_months):
    ...
    writer = multiple_dfs(setup_depo,"Depo",filename,2,5,list_rozvoz,list_zvoz,list_months)
    return writer
...
def write_emp(emp,writer):
    ...
    header_emp = ["Meno vodiča","Mesiac výkonu","Počet stopov rozvoz", "Počet stopov zvoz","Hmotnosť obj. rozvoz","Hmotnosť obj. zvoz","Počet doručených stopov","Počet zvezených stopov", "Σ (Rozvoz + zvoz)","% úsp. rozvoz","% úst. zvoz"]
    text_format = workbook.add_format({'text_wrap': True,"bold": True, 'font_color': '#f1f1f1',"bg_color":"#303030","valign":"vcenter","align":"center"})
...
def parse_file(filename):
    bar["value"] = 10
    root.update()
    spec_columns = ["Meno vodiča","Dátum výkonu","počet doručených stopov","hmotnosť doručených objednávok","počet zvezených stopov","hmotnosť zvezených objednávok","počet stopov rozvoz","počet stopov zvoz"]
    excel_data_df = pandas.read_excel(filename,usecols=spec_columns,dtype={'Meno vodiča': str,'počet doručených stopov': int,'hmotnosť doručených objednávok': float,'počet zvezených stopov': int,'hmotnosť zvezených objednávok': float,'počet stopov rozvoz': int,'počet stopov zvoz': float, })
