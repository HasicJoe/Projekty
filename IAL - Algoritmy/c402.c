...
**/
	...
	if(*RootPtr == NULL){
		...
		*RootPtr = (tBTNodePtr)malloc(sizeof(struct tBTNode));
		...
		tBTNodePtr root = (*RootPtr); //ulozime si koreň
		tBTNodePtr movingPtr; // pointer na pohybovanie sa stromom
		...
		while(notInserted){
			...
			movingPtr = (*RootPtr); // ulozime si predchodcu pred posunutim sa
			...
			if( (*RootPtr)->Cont == Content){
				...
				(*RootPtr) = (*RootPtr)->RPtr; // posunieme sa do pravej vetvy
				
				if( (*RootPtr) == NULL){
					notInserted = false;
					leftRight = 2; // Rptr move
				}
			}
			
			else if( (*RootPtr)->Cont > Content){
				
				(*RootPtr) = (*RootPtr)->LPtr; //posunieme sa do lavej vetvy
				
				if( (*RootPtr) == NULL){
					notInserted = false;
					leftRight = 1; // Lptr move
				}
			}
		}

		if(isRoot){
			(*RootPtr) = root;	//hodnota sa uz nachadzala v strome ziadna zmena
			return;
		}

		tBTNodePtr insertNew = (tBTNodePtr)malloc(sizeof(struct tBTNode));
		
		if(insertNew != NULL){
		
		insertNew->Cont = Content;
		insertNew->LPtr = NULL;
		insertNew->RPtr = NULL;
		}

		if(leftRight == 1) {
			movingPtr->LPtr = insertNew;
		}
		else if(leftRight == 2 ) {
			movingPtr->RPtr = insertNew;
		}

		(*RootPtr) = root;	//vratime opat ukazatel na koreň
	}
}
...
**/
	while(ptr != NULL){
		...
		SPushP(Stack,ptr);	// ulozime ukazatel na vrchol zasobnika
		BTWorkOut(ptr);		// spracujeme uzol
		ptr = ptr->LPtr;	// posunieme sa dolava
	}
}
...
**/
	if(RootPtr != NULL){
		...
		tStackP Stack;
		SInitP(&Stack);
		Leftmost_Preorder(RootPtr,&Stack);	// prejdeme lave uzly rootpointeru

		while(SEmptyP(&Stack) == false){

			RootPtr = STopPopP(&Stack);
			RootPtr = RootPtr->RPtr;	// posunieme sa doprava
			Leftmost_Preorder(RootPtr,&Stack);	// prejdeme vsetky lave uzly ukazatela
		}
	}
}
...
**/
	if(RootPtr != NULL){
		tStackP Stack;
		SInitP(&Stack);
		Leftmost_Inorder(RootPtr,&Stack);	// prejdeme lave uzly rootPointru

		while(SEmptyP(&Stack) == false){

			RootPtr = STopPopP(&Stack);	
			BTWorkOut(RootPtr);
			RootPtr = RootPtr->RPtr;  // lavy uzol a otca sme spracovali ostal uz len pravy syn
...
**/
	while(ptr != NULL){
		...
		SPushP(StackP,ptr);
		SPushB(StackB,true);	//budeme sa vracat z lava (true)
		ptr = ptr->LPtr;
	}
}
...
**/
	if(RootPtr != NULL){
		...
		while(SEmptyP(&StackP) == false){
			...
			from_left = STopPopB(&StackB);	//vezmeme si hodnotu z vrcholu bool stacku

			if(from_left == true){

				SPushB(&StackB,false);
				SPushP(&StackP,RootPtr);
				RootPtr = RootPtr->RPtr;	// ideme navstivit praveho syna
				Leftmost_Postorder(RootPtr,&StackP,&StackB);
			}
			else{
				BTWorkOut(RootPtr);
...
// pre inspiraciu posluzila stranka: https://www.techiedelight.com/deletion-from-bst/
	...
	if(*RootPtr != NULL){
		...
		while(SEmptyP(&Stack) == false){
			...
			tmp = STopPopP(&Stack);	//vytiahneme z vrcholu zasobnika

			if(tmp->LPtr != NULL){
				SPushP(&Stack,tmp->LPtr);	// ak ma laveho syna ulozime do zasbonika
			}

			if(tmp->RPtr != NULL){
				SPushP(&Stack,tmp->RPtr);	// ak ma praveho syna tak ulozime do zasobnika
			}
			free(tmp);
		}
		*RootPtr = NULL; // inicializacia
	}
}
